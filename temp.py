...
def display_tables(branch):
    ...
    if table_names:
        for i, table in enumerate(table_names):
            label = tk.Label(inner_frame, text=table[0], bg='white')
        ...
        label = tk.Label(inner_frame, text=f"No tables found for {branch} branch", bg='white')
...
def show_context_menu(event, t):
    if t.endswith('_cln'):
        ...
        context_menu = tk.Menu( inner_frame, tearoff=0 )
        m5=tk.Menu(inner_frame , tearoff=0)
...
display_button.pack(side=tk.TOP, pady=5)
